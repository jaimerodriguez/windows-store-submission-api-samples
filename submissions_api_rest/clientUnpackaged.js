...
config.applicationId = config.unpackagedApplicationId; // quick way to reuse same config object for both packaged and unpackaged samples since the only difference is applicationId
...
if ( !result || !result.responseData || !result.responseData.isSuccess){
    print ("Error in polling function. Exiting since this is a brief sample"); 
    print (JSON.stringify(result || {"emptyResult": true }));
