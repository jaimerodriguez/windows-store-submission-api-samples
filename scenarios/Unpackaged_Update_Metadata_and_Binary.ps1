...
# No need (and no API) to delete ongoing submission, but be aware draft submissions could be present.
...
$updatedMetadataJSON= @"
{
    ...
    {
        "language":"en",
        "description":"description for $updatePostfix",
        "whatsNew":"new stuff for $updatePostfix",
        "productFeatures":["$updatePostfix rocks","$updatePostfix is funny too"],
...
$spanishUpdateMetadata=@"
{
    "listings":
    {
        "language":"es",
        "description":"Descripcion de $updatePostfix",
        "whatsNew":"Nada nuevo en $updatePostfix",
        "productFeatures":["Diversion","inteligencia artificial"],
...
# Process metadata for both languages
$metadataTemplates = @($updatedMetadataJSON, $spanishUpdateMetadata)
$languages = @("en", "es")

for ($i = 0; $i -lt $metadataTemplates.Length; $i++) {
    $currentTemplate = $metadataTemplates[$i]
    $currentLanguage = $languages[$i]

    Write-Host "Processing metadata for language: $currentLanguage" -ForegroundColor Cyan

    $updatedMetadata = $currentTemplate | ConvertFrom-Json -Depth 50 | ConvertTo-Json -Depth 50


    msstore submission updateMetadata $AppId $updatedMetadata --verbose
    if ( $LASTEXITCODE -ne 0 ) {
        Write-Host "Failed to update $currentLanguage metadata. Exiting..." -ForegroundColor Red
        exit 1
    } else {
        Write-Host "Successfully updated $currentLanguage metadata" -ForegroundColor Green
    }
}
